...
class VMDCNNTransformer(nn.Module):
    def __init__(self, input_dim, conv_archs, hidden_dim, num_layers, num_heads, output_dim, output_size,
                 dropout_rate=0.5):
    ...
    def forward(self, input_seq):
        ...
        input_seq = input_seq.permute(0, 2, 1)  # torch.Size([64, 9, 24])
        cnn_features = self.cnn_features(input_seq)

        cnn_features = cnn_features.permute(0, 2, 1)

        transformer_features = self.transformer(cnn_features)  # torch.Size([64, 6, 64])

        output_avgpool = self.avgpool(transformer_features.transpose(1, 2))
        output = output_avgpool.reshape(batch_size, -1)
        predict = self.linear(output)  # torch.Size([64, 3]
        return predict
...
model = VMDCNNTransformer(input_dim, conv_archs, hidden_dim, num_layers, num_heads, output_dim, output_size)
...
def model_train(epochs, model, optimizer, loss_function, train_loader, test_loader, device):
    ...
    torch.save(best_model, 'best_model_vmd_cnn_transformer.pt')
...
model = torch.load('best_model_vmd_cnn_transformer.pt')
model = model.to(device)
...
model = torch.load('best_model_vmd_cnn_transformer.pt',weights_only=False)
model = model.to(device)
...
testset = load('test_xdata')  # torch.Size([3458, 24, 9])
my_data = testset[-1]  # torch.Size([24, 9])
...
plt.plot(pre_data[:, step], label=f'VMD+CNN-Transformer model T + {step + 1} step predict',color='green')
plt.legend()
plt.show()