...
class DecoderWithCrossAttention(nn.Module):
    ...
    def forward(self, x, hidden, cell, encoder_outputs):
        ...
        weighted, attention_weights = self.cross_attention(hidden[-1], encoder_outputs)
...
class Seq2SeqLSTMModelWithCrossAttention(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, output_dim, pre_len):
        super(Seq2SeqLSTMModelWithCrossAttention, self).__init__()
        self.encoder = Encoder(input_dim, hidden_dim, num_layers)
        self.decoder = DecoderWithCrossAttention(hidden_dim, output_dim, num_layers)
...
model = Seq2SeqLSTMModelWithCrossAttention(input_dim, hidden_dim, num_layers, output_dim, pre_len)
...
def model_train(epochs, model, optimizer, loss_function, train_loader, test_loader, device):
    ...
    torch.save(best_model, 'best_model_Seq2SeqLSTMModelWithAttention.pt')
...
model = torch.load('best_model_Seq2SeqLSTMModelWithAttention.pt')
print(type(model))
model = model.to(device)
...
model = torch.load('best_model_Seq2SeqLSTMModelWithAttention.pt',weights_only=False)
model = model.to(device)
scaler = load('scaler')
