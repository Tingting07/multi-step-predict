...
class CNNLSTMModel(nn.Module):
    ...
    def forward(self, input_seq):
        ...
        lstm_out = cnn_features.permute(0, 2, 1)  # torch.Size([64, 6, 64])
        for lstm in self.lstm_layers:
            lstm_out, _ = lstm(lstm_out)
        # print(lstm_out.size())  # torch.Size([64, 6, 128])
        predict = self.linear(lstm_out[:, -1, :])  # torch.Size([64, 3]
        return predict
...
model = CNNLSTMModel(batch_size, input_dim, conv_archs, hidden_layer_sizes, output_dim, output_size)
